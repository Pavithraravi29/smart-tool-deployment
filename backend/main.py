        ...
        async with app.state.postgres_pool.acquire() as connection:
            ...
            if results:
                ...
                for i in range(0, len(results), batch_size):
                    ...
                    serialized_data = json.dumps(data, cls=DecimalEncoder)
                    await websocket.send_text(serialized_data)
                    await asyncio.sleep(3)  # Delay between sending each batch
            else:
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="172.18.101.47", port=8000)

# TO RUN
# uvicorn main:app --reload --host 172.18.101.47 --port 8000
